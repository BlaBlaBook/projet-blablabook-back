...
export async function registerUser(req: Request, res: Response) {
  ...
  const { username, email, password, confirmPassword } = await registerUserBodySchema.parseAsync(req.body);
...
export async function updateCurrentUser(req: Request, res: Response) {
  ...
  const { username, email, first_name, last_name, password, confirmPassword } = await updateUserBodySchema.parseAsync(req.body);
  
  // If email is being updated, check if it's already taken
  if (email) {
    const existingUser = await prisma.users.findFirst({ where: { email, id: { not: userId } } });
